...
/**
 * Output a color hex code given a 0-1 float. 0 is green, 1 is red. Interpolation is not disgusting (it goes thru yellow and orange instead of weird brown).
 */
export function greenToRed(value: number) {
	const green = [0, 255, 0];
	...
	let interpColor;
	if (value < 0.25) {
		interpColor = green.map((c, i) => Math.round(c + (yellow[i] - c) * (value * 4)));
	} else if (value < 0.5) {
		interpColor = yellow.map((c, i) => Math.round(c + (orange[i] - c) * ((value - 0.25) * 4)));
	} else {
		interpColor = orange.map((c, i) => Math.round(c + (red[i] - c) * ((value - 0.5) * 2)));
	}

	return '#' + interpColor.map((c) => c.toString(16).padStart(2, '0')).join('');
}
