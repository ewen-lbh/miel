...
<script lang="ts">
	...
	import IconClear from '~icons/msl/cancel-outline';
	const emit = createEventDispatcher<{ input: string | undefined }>();
	...
	let optionsWithDisplay: Array<readonly [string, string]> = [];
	$: optionsWithDisplay = Array.isArray(options)
		? options.map(
				(option) => (Array.isArray(option) ? option : [option, option]) as readonly [string, string]
			)
